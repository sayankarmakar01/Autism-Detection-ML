...
{ $process_id=date("Ymd_His")."_".mt_rand(100,999);
  ...
  if($age_month>18){$level1_sts="y";}else{$level1_sts="n";}
  ...
  { $ans=$_POST['group'.$l2_id];
    ...
    $qry=$mysqli->prepare("select question, yes, no from level_1_question where id=?");
    ...
    $qry=$mysqli->prepare("insert into result_question_1 (question, answer, correct, process_id) values(?, ?, ?, ?)");
  ...
  $qry->bind_param('ssssss',$score, $level, $process_id, $user_id, $out_of_score,$level1_sts);
  $qry->execute();

  if($level1_sts=="y")
  { header('location:level2.1');
  }
  else
...
<body>
    <h3>Developmental Questions Level 1</h3>
    <form action="level1" method="post">
        <!-- Question 1 -->
         <?php
            $qry=$mysqli->prepare("select id, question from level_1_question order by id");
            $qry->execute();
